...
export class AnimalRepository extends Repository<Animal> {
  ...
  async getPagesNumber(limit: number, search: string): Promise<number> {
    ...
    try {
      ...
      if (search !== '') {
        animalsAfterSearch = animalsLength.filter((animal) => {
          ['name'].some((key) => {
            if (
              animal[key].toLowerCase() == 'čáp' ||
              animal[key].toLowerCase() == 'Čáp'
            ) {
          ...
          return ['name', 'latinname'].some((key) =>
            animal[key].toLowerCase().includes(search.toLowerCase()),
          );
        });
  ...
  async getAll(page: number, limit: number, search: string): Promise<Animal[]> {
    ...
    try {
      let animals = await query.getMany();

      if (search !== '') {
        animals = animals.filter((animal) =>
          ['name', 'latinname'].some((key) =>
            animal[key].toLowerCase().includes(search.toLowerCase()),
          ),
        );
      }
